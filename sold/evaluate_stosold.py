...
from train_stosold import StoSOLDModule
...
@torch.no_grad()
def play_episode(sold: StoSOLDModule, mode: str = "eval") -> Dict[str, Any]:
...
@torch.no_grad()
def rollout(sold: StoSOLDModule, episode: Dict[str, Any]):
...
@hydra.main(config_path="../configs", config_name="evaluate_stosold")
def evaluate(cfg: DictConfig):
    ...
    for i, checkpoint in enumerate(tqdm(checkpoint_files, disable=len(checkpoint_files) == 1, desc="Evaluating checkpoints")):
        ...
        sold = StoSOLDModule.load_from_checkpoint(checkpoint, env=env)
