...
dmcgb_photometric_modes = [
    'color_easy', 'color_hard', 'video_easy', 'video_hard', 'color_video_easy', 'color_video_hard',
    'color_easy_train', 'color_hard_train', 'video_easy_train', 'video_hard_train', 'color_video_easy_train', 'color_video_hard_train',
    'color_easy_test', 'color_hard_test', 'video_easy_test', 'video_hard_test', 'color_video_easy_test', 'color_video_hard_test',
]
dmcgb_data_dir = "/project/sold/sold/envs/data"
...
# -------------------------------Color helpers------------------------------------
    ...
    def _load_colors(self):
        if 'hard' in self._mode:
            _colors = torch.load(f'{dmcgb_data_dir}/color_hard.pt', weights_only=False)
            if 'train' in self._mode:
                self._colors = _colors[:int(len(_colors) * 0.8)]
            elif 'test' in self._mode:
                self._colors = _colors[int(len(_colors) * 0.8):]
            else:
                self._colors = _colors
        elif 'easy' in self._mode:
            _colors = torch.load(f'{dmcgb_data_dir}/color_easy.pt', weights_only=False)
            if 'train' in self._mode:
                self._colors = _colors[:int(len(_colors) * 0.8)]
            elif 'test' in self._mode:
                self._colors = _colors[int(len(_colors) * 0.8):]
